...
| 文件 | 适用平台 |
| --- | --- |
| `DeepSeekBalanceWidget-v0.6.0-macos-arm64.zip` | macOS Apple Silicon（M 系列） |
...
- 通过官方用量接口持续显示 **OpenCode Go 额度**：5 小时 / 周 / 月三窗口剩余百分比、距恢复倒计时与进度条；未配置 Key / Key 无效 / 网络失败等状态直接显示在区块上（替代原 WorkBuddy 占位）。v0.6.0 起支持**双账号**：第二把 Key 独立配置，胶囊并排 OC1/OC2 两张迷你卡，详情卡按账号分组
...
- **OpenCode Go 双账号监测**：设置页可配置第二把 API Key（独立钥匙串条目），胶囊并排显示 OC1/OC2 两张迷你卡，详情卡按账号分组，菜单栏新增独立 OC2 状态项，账号2 预警独立评估
- **菜单栏信息量升级**：GPT 段追加 5 小时恢复倒计时（`GPT 65/79% 4h24m`），OC 标签改为 `OC1`；胶囊月额度行改显示恢复天数
- **GPT 预警事件日志**：低量预警与恢复提醒在评估触发时即落盘，用户关掉弹窗后仍可回溯「提醒到底触发过没有」
